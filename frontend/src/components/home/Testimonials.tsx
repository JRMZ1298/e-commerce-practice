...
const TestimonialCard = ({
  ...
  <div className="rounded-3xl p-6 bg-cards mb-4 flex-shrink-0 boty-shadow">
    <div className="flex gap-1 mb-3">
      {Array.from({ length: testimonial.rating }).map((_, i) => (
        <Star key={i} className="w-4 h-4 fill-brand-gold text-brand-gold" />
      ))}
    </div>
    <p className="text-foreground leading-relaxed mb-4 text-pretty font-medium text-[1.8rem] tracking-wide">
      &ldquo;{testimonial.text}&rdquo;
    </p>
    <div className="flex items-start justify-between gap-2">
      <div>
        <p className="text-foreground text-[1.4rem] font-bold">
          {testimonial.name}
        </p>
...
export function Testimonials() {
  ...
  return (
    <section className="py-24 bg-background overflow-hidden pb-24 pt-12 relative">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-b from-background to-[#5D4037] pointer-events-none" />
    </section>
  );
}
