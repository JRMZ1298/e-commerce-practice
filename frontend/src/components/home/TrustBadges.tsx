...
export function TrustBadges() {
  ...
  return (
    <section className="py-20 bg-background relative">
      <div className="mx-auto px-6 lg:px-8">
        <div ref={sectionRef} className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {badges.map((badge, index) => (
            <div
              key={badge.title}
              className={`bg-cards p-6 lg:p-8 text-center rounded-xl transition-all duration-700 ease-out boty-shadow ${
                isVisible
            ...
            >
              <badge.icon
                className="text-brand-accent mb-4 mx-auto size-12"
                strokeWidth={1}
              />
              <h3 className="text-foreground mb-2 text-[2rem] font-semibold">
                {badge.title}
              </h3>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-b from-background to-muted pointer-events-none" />
    </section>
  );
}
