...
export function AuthForm() {
  ...
  const loginFields: FieldState[] = [
    { name: 'email', label: 'Correo electrónico', type: 'email', autoComplete: 'email', isVisible: true },
    { name: 'password', label: 'Contraseña', type: showPassword ? 'text' : 'password', autoComplete: 'current-password', isVisible: true },
  ]

  const registerFields: FieldState[] = [
    { name: 'firstName', label: 'Nombre', type: 'text', autoComplete: 'given-name', isVisible: !isLogin && !transitioning },
    { name: 'lastName', label: 'Apellido', type: 'text', autoComplete: 'family-name', isVisible: !isLogin && !transitioning },
    { name: 'email', label: 'Correo electrónico', type: 'email', autoComplete: 'email', isVisible: true },
    { name: 'password', label: 'Contraseña', type: showPassword ? 'text' : 'password', autoComplete: isLogin ? 'current-password' : 'new-password', isVisible: true },
    { name: 'confirmPassword', label: 'Confirmar contraseña', type: showConfirm ? 'text' : 'password', autoComplete: 'new-password', isVisible: !isLogin && !transitioning },
  ]
  ...
  return (
    <div className="w-full max-w-[380px] space-y-5">
      <div className="text-left">
        ...
        <p className="mt-1 text-[1.3rem] text-foreground-muted">
          {isLogin
            ...
            {visibleFields.map((field) => (
              <div
                ...
                style={{
                  ...
                  transform: field.isVisible ? 'translateY(0)' : 'translateY(-8px)',
                  maxHeight: field.isVisible ? '72px' : '0',
                  overflow: 'hidden',
              ...
              >
                ...
                <div className="relative">
                  <input
                    ...
                    className="h-[44px] w-full rounded-xl border border-border bg-background px-4 text-[1.4rem] text-foreground placeholder:text-foreground-muted/50 focus:border-brand-accent focus:outline-none focus:ring-1 focus:ring-brand-accent"
        ...
        {isLogin && (
          <div className="text-right">
            <Link
              ...
              className="text-[1.2rem] text-foreground-muted transition-colors hover:text-brand-accent"
            >
      ...
      <div className="text-center pt-1">
        <button
          ...
          className="text-[1.3rem] font-normal text-foreground-muted transition-colors hover:text-foreground"
        >
