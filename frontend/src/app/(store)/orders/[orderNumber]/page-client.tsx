...
import {
  Package, ChevronLeft, ShoppingBag, Clock, CheckCircle, XCircle, Truck,
  ...
  if (isError || !order) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
        ...
        <Link href="/orders" className="btn-primary mt-6 px-8 py-4 text-[1.4rem]">
          Mis pedidos
        </Link>
      </div>
    )
      ...
      <div className="mx-auto max-w-col-lg">
        ...
        {isSuccess && order.status === 'AWAITING_PAYMENT' && (
          <div className="mb-6 rounded-xl bg-green-50 p-6 text-center">
            ...
            <p className="mt-1 text-[1.4rem] text-green-600">
              Tu pedido {order.orderNumber} ha sido registrado. Te notificaremos cuando sea confirmado.
            </p>
          </div>
        )}
        ...
        <div className="card-starbucks mb-6 p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className={cn('inline-flex w-fit items-center gap-1.5 rounded-full px-4 py-1.5 text-[1.3rem] font-medium', status.color)}>
              {status.icon}
              {status.label}
            </span>
          </div>
        ...
        {order.status === 'AWAITING_PAYMENT' && (
          <div className="mb-6 flex justify-end">
            <button
              ...
              className="btn-outline flex items-center gap-2 px-6 py-3 text-[1.3rem] text-destructive hover:bg-red-50 disabled:opacity-50"
            >
          ...
          <div className="space-y-6">
            ...
            {order.notes && (
              <div className="card-starbucks p-6">
                ...
                <p className="text-[1.4rem] text-muted-foreground">{order.notes}</p>
              </div>
            )}
              ...
              <div className="card-starbucks p-6">
                ...
                <div className="space-y-4">
                  {order.statusHistory.map((entry) => {
                    const entryStatus = statusConfig[entry.toStatus] ?? statusConfig.AWAITING_PAYMENT
                    return (
                      <div key={entry.id} className="flex gap-3">
                        <div className="mt-0.5 flex-shrink-0">
                          <span className={cn('flex h-6 w-6 items-center justify-center rounded-full', entryStatus.color)}>
                            {entryStatus.icon}
                          </span>
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-foreground">{entryStatus.label}</p>
                          ...
                          {entry.notes && (
                            <p className="mt-0.5 text-[1.2rem] text-muted-foreground">{entry.notes}</p>
                          )}
                        </div>
                      </div>
                    )
