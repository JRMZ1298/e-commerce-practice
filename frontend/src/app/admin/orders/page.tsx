...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        <h1 className="font-serif text-[2rem] sm:text-[2.4rem] font-bold text-foreground">Pedidos</h1>
            ...
            {orders.map((order) => {
              ...
              return (
                <Link
                  key={order.id}
                  href={`/admin/orders/${order.orderNumber}`}
                  className="flex items-center justify-between rounded-xl bg-white p-4 boty-shadow transition-all hover:shadow-md"
                >
                  <div className="min-w-0 flex-1">
                    <p className="font-mono font-medium text-foreground">{order.orderNumber}</p>
                    <p className="mt-0.5 text-[1.2rem] text-muted-foreground">{customerName}</p>
                    <div className="mt-2 flex items-center gap-2">
                      <span className="text-[1.3rem] font-semibold text-foreground">
                        {formatPrice(order.total)}
                      </span>
                      <span className={cn('inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[1rem] font-medium', cfg.color)}>
                        {cfg.label}
                      </span>
                    </div>
                  </div>
